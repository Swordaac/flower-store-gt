  ...
  useEffect(() => {
    const fetchBestSellers = async () => {
      try {
        ...
        setError(e instanceof Error ? e.message : 'Failed to load best sellers')
      } finally {
        setLoading(false)
      }
    }
    ...
    <div className="min-h-screen" style={{ backgroundColor: theme.colors.background }}>
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          ...
          <p style={{ color: theme.colors.text.primary }}>Our most popular arrangements, loved by customers.</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p style={{ color: theme.colors.text.primary }}>No best sellers found.</p>
          </div>
        ) : (
                  ...
                  <div className="mt-4">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1">
                      {product.productTypes?.slice(0, 2).map(pt => (
                        <span key={pt._id} className="text-xs px-2 py-1 rounded-full" style={{ backgroundColor: pt.color + '20', color: pt.color }}>
                          {pt.icon} {pt.name}
                        </span>
                      ))}
