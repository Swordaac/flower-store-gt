...
interface ProductType { _id: string; name: string; slug?: string; color?: string; icon?: string; }
interface Occasion { _id: string; name: string; slug?: string; color?: string; icon?: string; }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Special case: best sellers
        const isBestSellers = slug === 'best-sellers' || slug === 'best-seller'
        ...
        const occasionsData = occasionsRes.ok ? await occasionsRes.json() : { data: [] }

        const allTypes: ProductType[] = typesData?.data || []
        const allOccasions: Occasion[] = occasionsData?.data || []

        const typeMatch = allTypes.find(t => (t.slug || t.name?.toLowerCase().replace(/[^a-z0-9]+/g, '-')) === slug)
        const occasionMatch = allOccasions.find(o => (o.slug || o.name?.toLowerCase().replace(/[^a-z0-9]+/g, '-')) === slug)

        // Determine title
        if (isBestSellers) {
          setTitle('Best Sellers')
        } else if (typeMatch) {
          setTitle(typeMatch.name)
        } else if (occasionMatch) {
          setTitle(occasionMatch.name)
        } else {
          setTitle('')
        }
        ...
        if (isBestSellers) {
          ...
          url += `&productTypes[]=${typeMatch._id}`
        } else if (occasionMatch) {
          url += `&occasions[]=${occasionMatch._id}`
        } else {
    ...
    <div className="min-h-screen" style={{ backgroundColor: theme.colors.background }}>
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-light mb-2" style={{ color: theme.colors.text.primary }}>{humanTitle || 'Collection'}</h1>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p style={{ color: theme.colors.text.primary }}>No products found.</p>
          </div>
        ) : (
