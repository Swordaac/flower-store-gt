...
/**
 * POST /api/products/validate-stock - Stock validation disabled (stock is infinite)
...
router.post('/validate-stock', async (req, res) => {
  try {
    ...
    // Stock validation disabled - always return success with requested quantities
    const results = items.map(item => ({
      productId: item.productId,
      selectedTier: item.selectedTier,
      requested: item.quantity,
      available: 999999,
      allowed: item.quantity,
      status: 'ok'
    }));

    return res.json({ success: true, data: { items: results } });
  } catch (error) {
    console.error('Error in validate-stock endpoint:', error);
